...
for i, cell in enumerate(nb.cells):
    if hasattr(cell, 'source'):
        ...
        if 'confusion_matrix' in src and 'savefig' in src and 'roc_auc_score' in src:
            print("Encontrada celda %d: EVALUACION EN TEST" % (i+1))
...
for i, cell in enumerate(nb.cells):
    if hasattr(cell, 'source'):
        ...
        if 'joblib.dump' in src and 'pipeline_match_predictor' in src:
            print("Encontrada celda %d: GUARDAR PIPELINE" % (i+1))
            print("Contenido actual:")
            print(src)
            break
...
faltantes = [imp for imp in imports_requeridos if imp not in cell_imports]
if faltantes:
...
print("Librerias importadas")'''
    ...
    print("Todos los imports ya estan presentes")

# 2. y 3. Corregir celda de guardar pipeline
for i, cell in enumerate(nb.cells):
    if hasattr(cell, 'source'):
        src = cell.source
        if 'joblib.dump' in src and 'pipeline_match_predictor' in src:
            print("Corrigiendo celda %d (GUARDAR PIPELINE)..." % (i+1))
